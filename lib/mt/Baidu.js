...
class Baidu extends MTClient {
	 ...
	 * Should change split format for Baidu Translate
	...
	translateText( sourceLang, targetLang, sourceText ) {
		...
		return preq.post( postData ).then( ( response ) => {
			...
			if (response.body.trans_result && Array.isArray(response.body.trans_result)) {
				let retLines = response.body.trans_result.map((result) => {
					return result.dst;
				});
